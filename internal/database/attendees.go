...
func (m *AttendeeModel) Insert(attendee *Attendee) (*Attendee, error) {
	...
	query := "INSERT INTO attendees (event_id, user_id) VALUES (?, ?) RETURNING id"

	err := m.DB.QueryRowContext(ctx, query, attendee.EventId, attendee.UserId).
		Scan(&attendee.Id)
	if err != nil {
		return nil, err
	}
...
func (m *AttendeeModel) GetByEventAndAttendee(eventId, userId int) (*Attendee, error) {
	...
	err := m.DB.QueryRowContext(ctx, query, eventId, userId).
		Scan(&attendee.Id, &attendee.UserId, &attendee.EventId)
